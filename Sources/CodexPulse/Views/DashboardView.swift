...
private struct UsageIntelligenceScreen: View {
    ...
    private var tokenEmptyState: some View {
        if ui.usageProviderFilter == .claude {
            EmptyDataCard(
                icon: "chart.bar.xaxis",
                title: tr(settings.language, "Ждём первую Claude Code-сессию", "Waiting for the first Claude Code session"),
                message: tr(
                    settings.language,
                    "Claude Web даёт лимиты, сбросы и прогноз темпа. После первой Claude Code-сессии Pulse локально покажет токены, модели и проекты — без сохранения текстов чатов.",
                    "Claude Web provides quotas, resets, and pace forecasts. After the first Claude Code session, Pulse will show local tokens, models, and projects without storing chat text."
                ),
            ...
            EmptyDataCard(
                icon: "cpu",
                title: tr(settings.language, "Собираем локальную историю Codex", "Collecting local Codex history"),
                message: tr(
                    settings.language,
                    "После первого успешного сканирования здесь появятся токены, модели и проекты. Pulse читает только агрегаты — без текстов чатов.",
                    "Tokens, models, and projects will appear here after the first successful scan. Pulse reads aggregates only, never chat text."
                ),
                actionTitle: tr(settings.language, "Обновить данные", "Refresh data")
            ) {
                Task { await state.refresh(forceAnalytics: true) }
...
private struct ModelUsageView: View {
        ...
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 10) {
                if models.isEmpty {
                    EmptyDataCard(
                        icon: "cpu",
                        title: tr(settings.language, "Модели пока не определены", "Models are not identified yet"),
                        message: tr(
                            settings.language,
                            "Pulse покажет разбивку после первого успешного локального сканирования. Тексты чатов при этом не читаются.",
                            "Pulse will show a breakdown after the first successful local scan. Chat text is not read."
                        ),
                        actionTitle: tr(settings.language, "Обновить данные", "Refresh data")
                    ) {
                        Task { await state.refresh(forceAnalytics: true) }
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(tr(settings.language, "На что ушли локальные токены", "Where local tokens went"))
                    ...
                    ForEach(models) { model in
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                HStack {
                                    Text(model.modelName).font(.headline)
                                    Spacer()
                                    Text(shareText(model.shareOfAttributedTokens)).font(.headline.monospacedDigit())
                                }
                                HStack {
                                    Text(compactTokens(model.totalTokens)).font(.callout.monospacedDigit())
                                    Text(tr(settings.language, "локальных токенов", "local tokens"))
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                                ProgressView(value: model.shareOfAttributedTokens).tint(modelColor(model.modelName))
                                if let project = model.topProjectName, let tokens = model.topProjectTokens {
                                    Label(
                                        tr(settings.language, "Больше всего: ", "Most in: ") + project + " · " + compactTokens(tokens),
                                        systemImage: "folder"
                                    )
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
    ...
    private var snapshots: [UsageAnalyticsSnapshot] { filteredAnalytics(state: state, filter: DashboardUIState.shared.usageProviderFilter) }
    private var breakdown: ModelUsageBreakdown { .combined(from: snapshots) }
    private var models: [ModelUsageInsight] { breakdown.insights }
    private var coverageDescription: String {
        if breakdown.hasCompleteCoverage {
            return tr(settings.language, "Доля — от всех локально учтённых токенов. Это не доля лимита ChatGPT Pro.", "Share is based on all locally attributed tokens. It is not a ChatGPT Pro quota share.")
        }
        return tr(
            settings.language,
            "Модель определена для \(shareText(breakdown.coverage)) локальных токенов. Это не доля лимита ChatGPT Pro.",
            "A model is known for \(shareText(breakdown.coverage)) of local tokens. It is not a ChatGPT Pro quota share."
        )
    }
    private func shareText(_ share: Double) -> String { "\(Int((share * 100).rounded()))%" }
