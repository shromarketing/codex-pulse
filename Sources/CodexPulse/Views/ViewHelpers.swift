...
func codexQuotaMeters(_ details: CodexAccountDetails, language: AppLanguage) -> [QuotaMeterItem] {
    details.quotaBuckets.filter { !isSparkBucket($0) }.flatMap { bucket -> [QuotaMeterItem] in
...
private func isSparkBucket(_ bucket: CodexQuotaBucket) -> Bool {
    bucket.id.localizedCaseInsensitiveContains("spark")
        || bucket.name?.localizedCaseInsensitiveContains("spark") == true
}
