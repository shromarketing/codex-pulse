...
struct FloatingWidgetView: View {
    ...
    @State private var showsResetSignals = false
    ...
    private var signalButton: some View {
        Button {
            showsResetSignals.toggle()
            if showsResetSignals {
                FloatingPanelController.shared.showSignals()
            }
        } label: {
            Image(systemName: showsResetSignals ? "chevron.left" : "bell.badge")
        }
        .buttonStyle(.plain)
        .help(showsResetSignals
              ? tr(settings.language, "Вернуться к квотам", "Back to quotas")
              : tr(settings.language, "Открыть сигналы сброса", "Open reset signals"))
        .accessibilityLabel(showsResetSignals
                            ? tr(settings.language, "Вернуться к квотам", "Back to quotas")
                            : tr(settings.language, "Открыть сигналы сброса", "Open reset signals"))
    }
    ...
    private var widgetProviderSubtitle: String {
        if showsResetSignals {
            return tr(settings.language, "Сигналы возможных сбросов", "Possible reset signals")
        }
...
func widgetPresentationForSignals(from layout: WidgetLayout) -> WidgetPresentation {
    switch layout {
    case .mini, .compact, .focus:
        .focus
    case .expanded:
        .adaptive
    }
