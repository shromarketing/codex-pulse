...
struct SettingsView: View {
    ...
    private var menuBarSection: some View {
        Section(tr(settings.language, "Строка меню", "Menu bar")) {
            ...
            Text(tr(settings.language, "В режиме «Процент» показаны фирменные значки Codex и Claude Code.", "Percent mode shows the official Codex and Claude Code icons."))
    ...
    private var analyticsSection: some View {
        Section(tr(settings.language, "Использование и расходы", "Usage and costs")) {
            ...
            LabeledContent(
                tr(settings.language, "Claude Code (локально)", "Claude Code (local)"),
                value: tr(settings.language, "Токены, модели и проекты после первой сессии", "Tokens, models, and projects after the first session")
            )
            Text(tr(
                settings.language,
                "Pulse локально хранит историю процентов каждого окна Codex и Claude. Claude Web передаёт только квоты; Claude Code добавляет локальные агрегаты токенов, моделей и проектов. Тексты задач не сохраняются.",
                "Pulse stores percentage history for every Codex and Claude window locally. Claude Web provides quotas only; Claude Code adds local token, model, and project aggregates. Task text is never stored."
            ))
                .font(.caption).foregroundStyle(.secondary)
        }
    }
    ...
    private var notificationsSection: some View {
        Section(tr(settings.language, "Оповещения", "Alerts")) {
            ...
            Toggle(tr(settings.language, "Мониторить анонсы сбросов Codex и Claude", "Monitor Codex and Claude reset announcements"), isOn: $settings.resetSignalChecksEnabled)
            if settings.resetSignalChecksEnabled {
                LabeledContent(tr(settings.language, "Проверка", "Checks"), value: tr(settings.language, "каждые 30 мин", "every 30 min"))
                Text(resetSignalStatusText)
                    .font(.caption)
                    .foregroundStyle(resetSignalStatusColor)
                if let sourceURL = state.resetSignalStatus.latestSignal?.sourceURL {
                    Link(tr(settings.language, "Открыть первоисточник Tibo", "Open Tibo's original post"), destination: sourceURL)
                        .font(.caption)
                }
                Text(tr(
                    settings.language,
                    "Pulse читает только публичную ленту Tibo через relay. Лимиты, аккаунт и история чатов никуда не отправляются. Уведомление — ранний сигнал, факт подтверждается лимитом в приложении.",
                    "Pulse reads only Tibo's public feed through a relay. Quotas, account data, and chat history are never sent. This is an early signal; the app confirms the actual reset from your quota."
                ))
                .font(.caption)
                .foregroundStyle(.secondary)
            }
    ...
    private var resetSignalStatusText: String {
        switch state.resetSignalStatus.state {
        case .idle:
            return tr(settings.language, "Первичная проверка ожидается", "Initial check is pending")
        case .disabled:
            return tr(settings.language, "Мониторинг выключен", "Monitoring is off")
        case .checked:
            return tr(settings.language, "Новых анонсов нет", "No new announcements")
        case .signal:
            return tr(settings.language, "Найден новый анонс сброса", "A new reset announcement was found")
        case .unavailable:
            return tr(settings.language, "Источник анонсов временно недоступен", "Announcement source is temporarily unavailable")
        }
    }

    private var resetSignalStatusColor: Color {
        switch state.resetSignalStatus.state {
        case .signal: .pulseGreen
