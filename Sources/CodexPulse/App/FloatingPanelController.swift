...
final class FloatingPanelController: NSObject, NSWindowDelegate {
    ...
    func showSignals() {
        let currentLayout = panel.map { WidgetLayout(size: $0.contentLayoutRect.size) } ?? .compact
        SettingsStore.shared.widgetPresentation = widgetPresentationForSignals(from: currentLayout)
        showAndFocus()
    }
