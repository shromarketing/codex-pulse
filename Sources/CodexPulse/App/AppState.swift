...
final class AppState: ObservableObject {
    ...
    @Published var resetSignalStatus = ResetSignalMonitorStatus.idle
    @Published var claudeResetSignalStatus = ResetSignalMonitorStatus.idle
    ...
    @Published var isCheckingResetSignals = false
    ...
    private let resetSignalMonitor = ResetSignalMonitor()
    private let claudeResetSignalMonitor = ClaudeResetSignalMonitor()
    ...
    private var lastResetSignalCheck: Date?
    ...
    func refresh(forceAnalytics: Bool = false) async {
        ...
        let resetSignalIsDue = settings.resetSignalChecksEnabled && (forceAnalytics
            || lastResetSignalCheck.map { Date.now.timeIntervalSince($0) > ResetSignalMonitor.pollInterval } != false)
        ...
        if resetSignalIsDue {
            await refreshResetSignals()
        } else if !settings.resetSignalChecksEnabled {
            resetSignalStatus = ResetSignalMonitorStatus(state: .disabled, checkedAt: nil, latestSignal: nil, recentSignals: [], message: nil)
            claudeResetSignalStatus = ResetSignalMonitorStatus(state: .disabled, checkedAt: nil, latestSignal: nil, recentSignals: [], message: nil)
        }
    ...
    func refreshResetSignals() async {
        guard settings.resetSignalChecksEnabled else {
            resetSignalStatus = ResetSignalMonitorStatus(state: .disabled, checkedAt: nil, latestSignal: nil, recentSignals: [], message: nil)
            claudeResetSignalStatus = ResetSignalMonitorStatus(state: .disabled, checkedAt: nil, latestSignal: nil, recentSignals: [], message: nil)
            return
        }
        guard !isCheckingResetSignals else { return }
        isCheckingResetSignals = true
        defer { isCheckingResetSignals = false }

        async let codexUpdate = resetSignalMonitor.poll()
        async let claudeUpdate = claudeResetSignalMonitor.poll()
        let (codex, claude) = await (codexUpdate, claudeUpdate)
        resetSignalStatus = codex.status
        claudeResetSignalStatus = claude.status
        lastResetSignalCheck = codex.status.checkedAt ?? claude.status.checkedAt
        if let signal = codex.signalToNotify {
            await NotificationService.shared.notify(resetSignal: signal, settings: settings)
        }
        if let signal = claude.signalToNotify {
            await NotificationService.shared.notify(resetSignal: signal, settings: settings)
        }
    }
