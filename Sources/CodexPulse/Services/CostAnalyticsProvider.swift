...
actor CostAnalyticsProvider {
    private let localCodexScanner = CodexSessionAnalyticsScanner()
    private let localClaudeCodeScanner = ClaudeCodeSessionAnalyticsScanner()

    func fetch(days: Int = 30) async -> [ProviderKind: UsageAnalyticsSnapshot] {
        async let codex = localCodexScanner.fetch(days: days)
        async let claude = localClaudeCodeScanner.fetch(days: days)
        let (nativeCodex, nativeClaude) = await (codex, claude)

        var result: [ProviderKind: UsageAnalyticsSnapshot] = [:]
        if let nativeCodex { result[.codex] = nativeCodex }
        if let nativeClaude { result[.claude] = nativeClaude }

        // Keep compatibility data as a last resort only. Native metadata has
        // priority, so an old CodexBar install can never overwrite it.
        let fallback = await fetchCodexBarFallback(days: days)
        for (provider, snapshot) in fallback where result[provider] == nil {
            result[provider] = snapshot
        }
        return result
    }

    /// Compatibility only. Pulse does not require or install CodexBar.
    private func fetchCodexBarFallback(days: Int) async -> [ProviderKind: UsageAnalyticsSnapshot] {
