...
/// A presentation-safe view of local model usage. It deliberately does not
/// attribute an account quota or subscription allowance to a model.
struct ModelUsageInsight: Hashable, Identifiable, Sendable {
    let modelName: String
    let totalTokens: Int64
    let shareOfAttributedTokens: Double
    let cost: Double?
    let topProjectName: String?
    let topProjectTokens: Int64?

    var id: String { modelName }
}

struct ModelUsageBreakdown: Hashable, Sendable {
    let insights: [ModelUsageInsight]
    let totalTokens: Int64
    let attributedTokens: Int64

    var coverage: Double {
        guard totalTokens > 0 else { return 0 }
        return min(1, Double(attributedTokens) / Double(totalTokens))
    }

    var hasCompleteCoverage: Bool {
        totalTokens == attributedTokens
    }

    static func combined(from snapshots: [UsageAnalyticsSnapshot]) -> ModelUsageBreakdown {
        var models: [String: (tokens: Int64, cost: Double, hasCost: Bool)] = [:]
        var projectModels: [String: [String: Int64]] = [:]
        var projectNames: [String: String] = [:]

        for snapshot in snapshots {
            for model in snapshot.modelTotals {
                var total = models[model.modelName] ?? (0, 0, false)
                total.tokens += model.totalTokens
                if let cost = model.cost {
                    total.cost += cost
                    total.hasCost = true
                }
                models[model.modelName] = total
            }

            for project in snapshot.projects {
                let projectID = snapshot.provider.rawValue + ":" + project.id
                projectNames[projectID] = project.name
                for model in project.modelBreakdowns {
                    projectModels[model.modelName, default: [:]][projectID, default: 0] += model.totalTokens
                }
            }
        }

        let attributedTokens = models.values.reduce(Int64(0)) { $0 + $1.tokens }
        let insights = models.map { name, total in
            let topProject = projectModels[name]?.max { $0.value < $1.value }
            return ModelUsageInsight(
                modelName: name,
                totalTokens: total.tokens,
                shareOfAttributedTokens: attributedTokens > 0 ? Double(total.tokens) / Double(attributedTokens) : 0,
                cost: total.hasCost ? total.cost : nil,
                topProjectName: topProject.flatMap { projectNames[$0.key] },
                topProjectTokens: topProject?.value
            )
        }
        ...
        return ModelUsageBreakdown(
            insights: insights,
            totalTokens: snapshots.reduce(0) { $0 + $1.totals.totalTokens },
...
struct UsageAnalyticsSnapshot: Codable, Hashable, Sendable {
    ...
    var modelUsageBreakdown: ModelUsageBreakdown {
        ModelUsageBreakdown.combined(from: [self])
    }
