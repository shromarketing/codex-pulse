...
struct FloatingWidgetLayoutTests {
    ...
    @Test("Signals from a compact widget open a readable Focus surface")
    func signalActionUsesReadableWidgetSize() {
        #expect(widgetPresentationForSignals(from: .mini) == .focus)
        #expect(widgetPresentationForSignals(from: .compact) == .focus)
        #expect(widgetPresentationForSignals(from: .focus) == .focus)
        #expect(widgetPresentationForSignals(from: .expanded) == .adaptive)
    }
}
